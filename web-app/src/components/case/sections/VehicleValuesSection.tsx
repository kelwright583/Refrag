  ...
  if (!assessments || assessments.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-[#D4CFC7] bg-[#FAFAF8] p-10 text-center space-y-4">
        <DollarSign className="w-8 h-8 text-slate/40 mx-auto" />
        <div>
          <p className="text-sm font-medium text-charcoal">No assessment started</p>
          <p className="text-xs text-slate mt-1">Create an initial motor assessment to capture vehicle values</p>
        </div>
        <button
          onClick={async () => {
            setCreating(true)
...
export function VehicleValuesSection({ caseId }: SectionProps) {
  return (
    <AssessmentBridgeSection caseId={caseId}>
      {(assessment) => (
        <MMCodesValuesTab assessment={assessment} onNavigate={() => {}} />
      )}
